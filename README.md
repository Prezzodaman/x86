...
I've also made a start on a Sound Blaster library. This is a huge deal, because it finally allows me to play samples alongside the main loop, in high quality! Even better, it can be used alongside Beeplib to combine beeps and digital samples. It can even mix up to 4 sounds at once, in real time, while the code is running. It's still a huge work-in-progress, but it's functional enough that it can be used for basic sound effects!
